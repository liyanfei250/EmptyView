...
/**
 * @author zhaoyunahi
 * 生成bitmap，Drawable的公共类，提供从view中获取bitmap，可以用于截图等功能
 * drawable 转bitmap
 */
public class SGDrawableHelper {
    private static final String TAG = SGDrawableHelper.class.getSimpleName();
    ...
    public static Bitmap createBitmapFromView(View view, int leftCrop, int topCrop, int rightCrop, int bottomCrop) {
        Bitmap originBitmap = SGDrawableHelper.createBitmapFromView(view);
    ...
    Drawable getVectorDrawable(Context context, @DrawableRes int resVector) {
        try {
            ...
            SGUILog.d(TAG, "Error in getVectorDrawable. resVector=" + resVector + ", resName=" + context.getResources().getResourceName(resVector) + e.getMessage());
            return null;
        }
    }
