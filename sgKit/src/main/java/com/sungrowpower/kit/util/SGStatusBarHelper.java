 ...
 * @date 2016-03-27
 */
public class SGStatusBarHelper {
  ...
  private static boolean supportTranslucent() {
    ...
    return !(SGDeviceHelper.isEssentialPhone() && Build.VERSION.SDK_INT < 26);
  }
  ...
  public static void translucent(Window window, @ColorInt int colorOn5x) {
    ...
    if (SGNotchHelper.isNotchOfficialSupport()) {
      handleDisplayCutoutMode(window);
    }

    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
      // 小米 Android 6.0 ，开发版 7.7.13 及以后版本设置黑色字体又需要 clear FLAG_TRANSLUCENT_STATUS, 因此还原为官方模式
      if (SGDeviceHelper.isFlymeLowerThan(8) || (SGDeviceHelper.isMIUI() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M)) {
  ...
  private static boolean isMIUICustomStatusBarLightModeImpl() {
    if (SGDeviceHelper.isMIUIV9() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
      return true;
    }
    return SGDeviceHelper.isMIUIV5() || SGDeviceHelper.isMIUIV6() ||
        SGDeviceHelper.isMIUIV7() || SGDeviceHelper.isMIUIV8();
  }
  ...
  public static boolean FlymeSetStatusBarLightMode(Window window, boolean light) {
    ...
    if (window != null) {
      ...
      if (SGDeviceHelper.isFlymeLowerThan(7)) {
        try {
      ...
      } else if (SGDeviceHelper.isFlyme()) {
        result = true;
      }
    }
    return result;
  ...
  private static void initStatusBarHeight(Context context) {
    ...
    if (sStatusBarHeight <= 0) {
      if (sVirtualDensity == -1) {
        sStatusBarHeight = SGDisplayHelper.dp2px(context, STATUS_BAR_DEFAULT_HEIGHT_DP);
      } else {
