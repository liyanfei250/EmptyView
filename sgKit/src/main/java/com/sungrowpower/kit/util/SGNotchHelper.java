...
public class SGNotchHelper {

  private static final String TAG = "SGNotchHelper";
  ...
  public static boolean has3rdNotch(Context context) {
    if (SGDeviceHelper.isHuawei()) {
      return hasNotchInHuawei(context);
    } else if (SGDeviceHelper.isVivo()) {
      return hasNotchInVivo(context);
    } else if (SGDeviceHelper.isOppo()) {
      return hasNotchInOppo(context);
    } else if (SGDeviceHelper.isXiaomi()) {
  ...
  private static Rect get3rdSafeInsetRect(Context context) {
    // 全面屏设置项更改
    if (SGDeviceHelper.isHuawei()) {
      boolean isHuaweiNotchSetToShow = SGDisplayHelper.huaweiIsNotchSetToShowInSetting(context);
  ...
  private static Rect getRectInfoRotation0(Context context) {
    Rect rect = new Rect();
    if (SGDeviceHelper.isVivo()) {
      // TODO vivo 显示与亮度-第三方应用显示比例
      rect.top = getNotchHeightInVivo(context);
      rect.bottom = 0;
    } else if (SGDeviceHelper.isOppo()) {
      // TODO OPPO 设置-显示-应用全屏显示-凹形区域显示控制
      rect.top = SGStatusBarHelper.getStatusbarHeight(context);
      rect.bottom = 0;
    } else if (SGDeviceHelper.isHuawei()) {
  ...
  private static Rect getRectInfoRotation90(Context context) {
    ...
    if (SGDeviceHelper.isVivo()) {
      ...
      rect.left = SGStatusBarHelper.getStatusbarHeight(context);
      rect.right = 0;
    } else if (SGDeviceHelper.isHuawei()) {
  ...
  private static Rect getRectInfoRotation180(Context context) {
    ...
    if (SGDeviceHelper.isVivo()) {
      ...
      rect.top = 0;
      rect.bottom = SGStatusBarHelper.getStatusbarHeight(context);
    } else if (SGDeviceHelper.isHuawei()) {
      int[] notchSize = getNotchSizeInHuawei(context);
      rect.top = 0;
  ...
  private static Rect getRectInfoRotation270(Context context) {
    ...
    if (SGDeviceHelper.isVivo()) {
      ...
      rect.right = SGStatusBarHelper.getStatusbarHeight(context);
      rect.left = 0;
    } else if (SGDeviceHelper.isHuawei()) {
      if (sHuaweiIsNotchSetToShow) {
  ...
  public static int getNotchHeightInXiaomi(Context context) {
    ...
    return SGDisplayHelper.getStatusBarHeight(context);
  }

  public static int getNotchWidthInVivo(Context context) {
    return SGDisplayHelper.dp2px(context, 100);
  }

  public static int getNotchHeightInVivo(Context context) {
    return SGDisplayHelper.dp2px(context, 27);
  }
  ...
  public static boolean needFixLandscapeNotchAreaFitSystemWindow(View view) {
    return (SGDeviceHelper.isXiaomi() || SGDeviceHelper.isVivo()) && SGNotchHelper.hasNotch(view);
  }
}
